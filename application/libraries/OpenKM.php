...
class OpenKM {
    ...
    var $server    = 'http://localhost:8080/OpenKM/';
	var $user      = 'okmAdmin';
	var $password  = 'admin';
	...
	var $okmSearch = 'services/OKMSearch?wsdl';
	var $token;
	var $OKMAuth;
	...
	function isValid($path)
	{
		...
		try{
            ...
            $r = array('status' => TRUE , 'result' => $response);				
	...
	function findByName($search)
	{
	    try{
		    ...
		    $this->OKMSearch = new SoapClient($this->server.$this->okmSearch);
            $response  = $this->OKMSearch->findByName(array('token' => $this->token, 'name' => $search));	
			...
			{
			   $r  =  array('status' => FALSE ,'result' => FALSE, 'message'  => 'Sory, data NOT FOUND..');
			}
		}catch (Exception $e) {
		    
            $r  =  array('status' => FALSE ,'result' => FALSE, 'message'  => 'Get Find By Name Failed '. $e->getMessage()); 			
		}
