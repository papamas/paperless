...
class Usul extends MY_Controller {
	...
	public function index()
	{
		...
		$data['usulTaspen']     =  $this->usul->getAllTaspen();
	...
	{
		$id   = $this->input->get('id');						
		$q    = $this->usul->getExcelTaspen($id);
		
		// creating xls file
		$now              = date('dmYHis');
		$filename         = "DAFTAR ANTRIAN USUL BERKAS TASPEN".$now.".xls";
		...
		$html  = 'DAFTAR ANTRIAN USUL BERKAS TASPEN';
		if($q->num_rows() > 0){
			$row = $q->row();
		$html .= '<table><tr><td  colspan=2>TANGGAL</td><td>'.$row->tgl_usul.'</td></tr>';
